    ...
    def update(self, dt):
        ...
        for entity, actor in self.entity_manager.pairs_for_type(Actor):
            if actor.shielded > 0:
                ...
                image = drawable.image
                centerx  = int(self.display_rect.centerx + position.x)
                centery  = int(self.display_rect.centery + position.y)
                pygame.draw.circle(self.display, (0, 0, 255), (centerx, centery), int(math.sqrt(image.get_width()*image.get_width() +image.get_height()*image.get_height())/3), 6)
                self.display.blit(image, self.screen_position(position, drawable.image))
    ...
    def update(self, dt):
        ...
        for entity, hurtvol in self.entity_manager.pairs_for_type(Hurtvolume):
            ...
            if hurtvol.shape["type"] == "circle":
                pass#pygame.draw.circle(self.display, (255, 255, 0), self.screen_position((x, y)), hurtvol.shape["radius"], 0)
        ...
        for entity, actor in self.entity_manager.pairs_for_type(Actor):
            ...
            if actor.config["hitshape"]["type"] == "circle":
                pass#pygame.draw.circle(self.display, (255, 0, 0), self.screen_position((x, y)), actor.config["hitshape"]["radius"], 0)
